...
int   AVPMC_CreateRecorderVideo(const unichar *filename,
                                uint width,
                                uint height,
                                float frameRate,
                                int format,
...
int   AVPMC_CreateRecorderImages(const unichar *filename,
                                 uint width,
                                 uint height,
                                 float frameRate,
                                 int format,
...
int   AVPMC_CreateRecorderPipe(const unichar *filename,
							   uint width,
							   uint height,
							   float frameRate,
							   int format,
...
bool  AVPMC_GetFinalFileURL(int handle, unichar *fileUrl, int length);
int   AVPMC_SetEncodedFrameLimit(int handle, uint limit);
void  AVPMC_SetTexturePointer(int handle, void *texture);
void  AVPMC_SetRenderBuffer(int handle, void *renderBuffer);
...
bool  AVPMC_GetContainerFileExtensions(int videoCodecIndex,
									   int audioCodecIndex,
									   unichar *extensions,
...
bool  AVPMC_GetFreeSpaceInBytesForPath(const unichar *path, uint64_t *bytes);
...
void  AVPMC_RequestPhotoLibraryAuthorisation(MCPhotoLibraryAccessLevel level,
											 MCRequestPhotoLibraryAuthorisationCallback callback);
...
void  AVPMC_UpdateAmbisonicWeights(MCAmbisonicSourceRef source,
								   float azimuth,
								   float elevation,
								   MCAmbisonicOrder ambisonicOrder,
								   MCAmbisonicChannelOrder channelOrder,
								   float *weights);
void  AVPMC_EncodeMonoToAmbisonic(MCAmbisonicSourceRef source,
								  float *inSamples,
								  int inOffset,
								  int inCount,
								  int numChannels,
