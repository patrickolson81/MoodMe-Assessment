...
namespace RenderHeads.Media.AVProMovieCapture
{
	public class Utils
	{
		...
		public static bool ShowInExplorer(string itemPath)
		{
			...
			else if (itemPath.StartsWith(FileScheme))
			{
				string path = itemPath.Substring(FileScheme.Length);
				url = FileScheme + URLEscapePathByPercentEncoding(Directory.GetParent(path).FullName);
			}
			else
			{
		...
		{
#if UNITY_WEBGL
			freeSpace = System.UInt64.MaxValue;
			return true;
#else
			...
			return NativePlugin.GetFreeSpaceInBytesForPath(folderName, out freeSpace);
#endif
		}
